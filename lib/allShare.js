...
var setWxShare = function(wx, share_data) {
    var s_data = share_data;

    $.getJSON(Config.getSiteUrl("common") + '/wx/ticket.jsonp?url=' + encodeURIComponent(location.href.split('#')[0]) + '&__c=?', function(data) {
        ...
        for(var i in s_data.wxSet) {
            wx_defaults[i] = s_data.wxSet[i];
        }
